...
export default function AddMoney() {
  ...
  const [pricing, setPricing] = useState({ gateway_fee: 0, gst_amount: 0, total_paid: 0 });

  useEffect(() => {
    walletService.getBalance().then((data) => setBalance(Number(data.balance || 0))).catch(() => {});
    paymentService.getPhonePePricing().then((data) => setPricing((current) => ({
      ...current,
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        ...
        setPricing({
          gateway_fee: Number(result.gateway_fee || 0),
          gst_amount: Number(result.gst_amount || 0),
            ...
            <div className="flex flex-wrap gap-2 mt-8">
              {QUICK_AMOUNTS.map((value) => (
                <button key={value} type="button" onClick={() => setAmount(String(value))}
                  className={`px-4 py-2 rounded-xl border font-bold ${Number(amount) === value ? 'bg-purple-600 text-white' : 'bg-white text-gray-700'}`}>
                  ₹{value}
                </button>
              ))}
            </div>
